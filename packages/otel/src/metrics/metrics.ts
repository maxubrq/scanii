...
export class SkaniiMetrics {
    private _exporter: SkaniiPrometheusExporter = new SkaniiPrometheusExporter({}, (err) => {
        if (err) {
            console.error(err);
        } else {
    ...
    constructor(
        ...
        this._meter = new SkaniiMetricsMeter(this.name, this.version, [this._exporter]);
    }
    ...
    /**
     * Records the HTTP request duration.
     */
    ...
    /**
     * Records the message processing duration.
     */
    ...
    /**
     * Counts the message processing error.
     */
    public countMessageProcessingError(status: number, method: string, path: string) {
